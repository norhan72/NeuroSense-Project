...
export const Results = () => {
  ...
  const sectionConfig = [
    {
      ...
      nameAr: 'الصوت والنُطق',
      nameEn: 'Voice and Speech',
      maxScore: 12
    },
    {
      key: 'Movement',
      icon: <Activity className="w-5 h-5" />,
      nameAr: 'الحركة والإحساس',
      nameEn: 'Movement and Sensation',
      maxScore: 15
    },
    {
      key: 'Vision',
  ...
  const generateMedicalAnalysis = (
    ...
    if ((results.Vision?.level === 'high' && results.Movement?.level === 'high' && results.Cognitive?.level === 'high') ||
        (results.Vision?.level === 'high' && results.Movement?.level === 'high' && results.totalScore >= 40) ||
        (visionPercentage >= 60 && movementPercentage >= 60 && cognitivePercentage >= 50)) {
    ...
    if ((results.Mood?.level === 'high' && results.Cognitive?.level === 'high' && results.totalScore >= 35) ||
    ...
    if ((results.Movement?.level === 'high' && results.Pain?.level === 'high') ||
        (results.Pain?.level === 'high' && (results.Mood?.level === 'medium' || results.Mood?.level === 'high')) ||
        (painPercentage >= 70 && movementPercentage >= 50)) {
    ...
    if (results.Voice?.level === 'high' && results.Movement?.level === 'high' && results.Cognitive?.level !== 'high') {
    ...
    if (results.Movement?.level === 'high' && (results.History?.level === 'high' || results.History?.level === 'medium')) {
              ...
              <Card className="p-6 bg-gradient-to-br from-primary/5 to-secondary/5 backdrop-blur-lg border-2 border-primary/20 shadow-lg">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="p-4 bg-card/50 rounded-lg border border-border/30">
                    ...
                    <p className="text-xl font-bold">{data.age} {t('results.years')}</p>
                  </div>

                  {/* Gender */}
                  {data.gender && (
                    <div className="p-4 bg-card/50 rounded-lg border border-border/30">
                      <p className="text-sm text-muted-foreground mb-2">{language === 'ar' ? 'الجنس' : 'Gender'}</p>
                  ...
                  {data.timestamp && (
                    <div className="p-4 bg-card/50 rounded-lg border border-border/30">
                      <p className="text-sm text-muted-foreground mb-2">{language === 'ar' ? 'تاريخ الإدخال' : 'Date'}</p>
                      <p className="text-lg font-semibold">
                        {new Date(data.timestamp).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                        })}
                ...
                {data.medicalHistory && (
                  <div className="mt-6 p-5 bg-card/60 rounded-lg border-2 border-accent/20">
                    ...
                    <p className="text-base leading-relaxed whitespace-pre-line">{data.medicalHistory}</p>
                  </div>
                )}
              </Card>
            )}
              ...
              <Card className="p-6 bg-gradient-to-br from-primary/10 to-secondary/10 backdrop-blur-lg border-primary/30">
                ...
                <div className="space-y-4">
                  {sectionConfig.map((section) => {
                    const sectionData = detailedResults[section.key as keyof DetailedResults] as { score: number; level: string };
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {sectionData.score} / {section.maxScore} {language === 'ar' ? 'نقطة' : 'points'} ({percentage}%)
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="w-full bg-muted rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${
                              sectionData.level === 'high' ? 'bg-destructive' :
                              sectionData.level === 'medium' ? 'bg-yellow-500' : 'bg-green-500'
                            }`}
            ...
            {medicalAnalysis && detailedResults && (
              <Card className="p-8 bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 backdrop-blur-lg border-2 border-blue-500/30 shadow-xl">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-3xl font-bold mb-2">
                        {language === 'ar' ? 'التقرير الطبي الذكي' : 'AI-Powered Medical Summary'}
                      </h3>
                ...
                <div className="space-y-8">
                  ...
                  {medicalAnalysis.suspectedConditions && medicalAnalysis.suspectedConditions.length > 0 && (
                  ...
                  <div className="p-6 bg-gradient-to-r from-primary/20 via-secondary/20 to-accent/20 rounded-xl border-2 border-primary/40 shadow-lg">
                    <h4 className="text-xl font-bold mb-4 flex items-center gap-3">
                      ...
                      {language === 'ar' ? 'التوصية الرئيسية والخطوات التالية' : 'Primary Recommendation & Next Steps'}
                    </h4>
                  ...
                  {medicalAnalysis.specificRecommendations && medicalAnalysis.specificRecommendations.length > 0 && (
                    <div className="p-6 bg-card/70 rounded-xl border-2 border-border/60 shadow-lg">
                      <h4 className="text-xl font-bold mb-4 flex items-center gap-3">
                        <AlertCircle className="w-6 h-6 text-secondary" />
                        {language === 'ar' ? 'توصيات إضافية مخصصة حسب الأقسام' : 'Additional Section-Specific Recommendations'}
                      </h4>
                      <ul className="space-y-4">
                        {medicalAnalysis.specificRecommendations.map((rec, index) => (
                          <li key={index} className="flex items-start gap-4 p-4 bg-card/50 rounded-lg border border-border/30">
